...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    server_addr.sin_port = htons(port); // номер порта 5000 (в сетевом порядке байт) к которому будет привязан сокет
    ...
    while (1) {
        ...
        for (int i = 0; i < MAX_CLIENTS; i++) {
            // active clients
            if (clients[i].fd > 0) {
                // монитор на чтение данные от клиента 
                FD_SET(clients[i].fd, &temp_read);
                max_fd = (max_fd > clients[i].fd) ? max_fd : clients[i].fd;
                
                // если у клиента есть данные для отправки на целевой сервер, добавляю целевой дескриптор в temp_write для прослушивания на возможность записать  
            ...
            if (clients[i].target_fd > 0) {
                // монитор на чтение данные от целевого сервера
                FD_SET(clients[i].target_fd, &temp_read);
                max_fd = (max_fd > clients[i].target_fd) ? max_fd : clients[i].target_fd;
                
                // если есть данные для отправки клиенту, добавляю его дескриптор в temp_write для прослушивания на возможность записать
        ...
        if (FD_ISSET(server_fd, &temp_read)) {
            ...
            if (client_fd > 0) {
                ...
                if (slot != -1) {
                    ...
                    printf("\nновое подключение от %s:%d\n", inet_ntoa(client_addr.sin_addr), ntohs(client_addr.sin_port));
                } else {
                    close(client_fd);
                    printf("\nпревышено максимальное количество клиентов. Клиент %s:%d отключен\n", inet_ntoa(client_addr.sin_addr), ntohs(client_addr.sin_port));
                }
            }
        }
        ...
        for (int i = 0; i < MAX_CLIENTS; i++) {
            ...
            if (FD_ISSET(clients[i].target_fd, &temp_write)) {
                if (clients[i].target_fd > 0 && clients[i].buf_len > 0) {
                    ...
                    // сдвиг оставшихся данных в буфере
